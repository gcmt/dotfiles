...
def menu(plants):
    ...
    for p in plants:
        dim = "#777"
        fg = "foreground='#ce7d86'" if p['elapsed_watering'] > p['target_watering'] else ""
        line = ""
    ...
    cmd = ['mouse-menu', '-format', 'p', '-width', 'auto', '-markup-rows', '-maxlines', '30']
    try:
        return run(cmd, input="\n".join(entries), stdout=PIPE, encoding='utf8').stdout.strip()
    ...
    with open(os.path.expanduser(path), "r") as f:
        for item in yaml.safe_load(f):
            yield {
                ...
                "elapsed_watering": (today - item["last_watering"]).days if item["last_watering"] else 0,
                "elapsed_fertilizing": (today - item["last_fertilizing"]).days if item["last_fertilizing"] else 0
            }
...
def print_label(plants):
    ...
    print(f"%{{A1:{script_path} -edit:}}%{{A3:{script_path} -menu:}}{label}%{{A}}%{{A}}")
